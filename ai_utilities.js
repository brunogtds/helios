...
function buildLLMPrompt() {
    ...
    const allHypothesesForLLM = Object.values(categorizedHypotheses || {}).flat();

    if (!allHypothesesForLLM.length) {
        console.warn("No hypotheses available for Narrator Agent prompt.");
    }
    ...
    const targetStdDev = Math.sqrt(
        targetData.map(x => Math.pow(x - targetMean, 2)).reduce((a, b) => a + b, 0) / targetData.length
    );
    ...
    fullDataset.headers.forEach((header, index) => {
        ...
        if (potentialDates.length / validData.length > 0.8) {
            ...
            if (numericData.length > 0) {
                ...
                stats.std = Math.sqrt(
                    numericData.map(x => Math.pow(x - stats.mean, 2)).reduce((a, b) => a + b, 0) / numericData.length
                );
    ...
    const createCategoryJson = (categoryHypotheses) => {
        ...
        const top10 = categoryHypotheses.slice(0, 10).map(h => ({
            ...
            test_type: h.Test_Type || null,
            comparison_scope: h.Comparison_Scope || null,
            reference_conditions: h.Reference_Conditions || null,
    ...
    const greaterThanJson = createCategoryJson(
        allHypothesesForLLM.filter(h => h.Operator === 'greater than')
    );

    const lessThanJson = createCategoryJson(
        allHypothesesForLLM.filter(h => h.Operator === 'less than')
    );

    const varianceJson = createCategoryJson(
        allHypothesesForLLM.filter(h => h.Operator === 'variance is higher')
    );
...
### Narrator Agent
In this section:
- Present yourself as data analysis specialist;
- Analyze the most important findings from the provided hypotheses.
- Explain each finding in accessible language and relate the explanation to the user's goal.
- Use your own knowledge to interpret the results, beyond translating them to acessible language. 
- Limit your text to 2 or 3 paragraphs (400 tokens at most).
