...
def _normalize_test_type(test_type, operator):
    operator = str(operator or '')
    is_variance = 'variance' in operator
    default_test = 'levene_test' if is_variance else 'one_sample_t_test'
...
def _direction_matches(op_type, left_value, right_value):
    if op_type == 'greater than':
        return left_value > right_value
    if op_type == 'less than':
        return left_value < right_value
    return False
...
def calculate_fitness_metrics(hypotheses_to_test_df, original_df, discretized_df):
    ...
    global_variance = original_df[dimension_col].var(ddof=1)
    ...
    for _, hypothesis in hypotheses_to_test_df.iterrows():
        ...
        rest_dataset = original_df.loc[~filter_mask]
        ...
        test_type = _normalize_test_type(hypothesis.get('Test_Type') or hypothesis.get('test_type'), op_type)
        ...
        rest_values = pd.to_numeric(rest_dataset[dimension], errors='coerce').dropna()
        ...
        if op_type in ['greater than', 'less than']:
            ...
            ref_var = global_variance
            pvalue = 1.0

            if test_type == 'one_sample_t_test':
                alternative = 'greater' if op_type == 'greater than' else 'less'
                try:
                    ttest_result = stats.ttest_1samp(sub_values, popmean=global_mean, alternative=alternative)
                    pvalue = float(ttest_result.pvalue) if not pd.isna(ttest_result.pvalue) else 1.0
                except Exception:
                    pvalue = 1.0
                metrics['Impact_Lift'] = _safe_ratio(sub_mean, global_mean)
                ...
                ref_mean = rest_values.mean()
                ref_var = rest_values.var(ddof=1)
                metrics['Impact_Lift'] = _safe_ratio(sub_mean, ref_mean)

                try:
                    if test_type == 'welch_t_test':
                        alternative = 'greater' if op_type == 'greater than' else 'less'
                        result = stats.ttest_ind(sub_values, rest_values, equal_var=False, alternative=alternative)
                        pvalue = float(result.pvalue) if not pd.isna(result.pvalue) else 1.0

                    elif test_type == 'mann_whitney_u_test':
                        alternative = 'greater' if op_type == 'greater than' else 'less'
                        result = stats.mannwhitneyu(sub_values, rest_values, alternative=alternative)
                        pvalue = float(result.pvalue) if not pd.isna(result.pvalue) else 1.0

                    elif test_type == 'kruskal_wallis_test':
                        result = stats.kruskal(sub_values, rest_values)
                        pvalue = float(result.pvalue) if _direction_matches(op_type, sub_mean, ref_mean) and not pd.isna(result.pvalue) else 1.0

                    elif test_type == 'anova_test':
                        result = stats.f_oneway(sub_values, rest_values)
                        pvalue = float(result.pvalue) if _direction_matches(op_type, sub_mean, ref_mean) and not pd.isna(result.pvalue) else 1.0

                except Exception:
                    pvalue = 1.0

            metrics['Reference_Mean'] = ref_mean
            metrics['Reference_Variance'] = ref_var
            ...
            ref_var = rest_values.var(ddof=1)
            metrics['Reference_Variance'] = ref_var
            metrics['Impact_Lift'] = _safe_ratio(sub_var, ref_var if not pd.isna(ref_var) and ref_var > 0 else global_variance)

            try:
                stat, p = stats.levene(sub_values, rest_values)
                p = float(p) if not pd.isna(p) else 1.0
            except Exception:
                p = 1.0

            comparison_var = ref_var if not pd.isna(ref_var) and ref_var > 0 else global_variance
            if sub_var > comparison_var:
                metrics['Significance_pValue'] = min(max(p / 2, 0.0), 1.0)
            else:
...
def format_results_table(df, all_condition_attributes):
    ...
    final_columns_order = [
        ...
        'is_significant', 'Dimension', 'Aggregation', 'Operator', 'Test_Type', 'source_method'
...
def calculate_fitness_metrics_welchs(hypotheses_to_test_df, original_df, discretized_df):
    ...
    for _, hypothesis in hypotheses_to_test_df.iterrows():
        ...
        dimension = hypothesis['Dimension']
        op_type = hypothesis['Operator']
        metrics['Test_Type'] = 'welch_t_test' if op_type in ['greater than', 'less than'] else 'levene_test'
