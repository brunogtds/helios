...
def update_ai_progress(job_id, **kwargs):
    if not job_id:
        return
    current = AI_PROGRESS.get(job_id, {}).copy()
    current.update(kwargs)
    AI_PROGRESS[job_id] = current
...
def canonical_hypo_key(h: dict):
    ...
    ref_conds = h.get("reference_conditions") or []

    conds_sorted = sorted([_canonical_condition_tuple(c) for c in conds])
    ref_conds_sorted = sorted([_canonical_condition_tuple(c) for c in ref_conds])
    ...
    return (
        ...
        str(h.get("test_type", "")),
        str(h.get("comparison_scope", "")),
        tuple(conds_sorted),
        tuple(ref_conds_sorted),
    )
...
def normalize_test_type(test_type, aggregation="mean"):
    if aggregation == "variance":
        default_test = "levene_test"
        allowed = set(VARIANCE_TEST_TYPES)
    else:
        default_test = "one_sample_t_test"
        allowed = set(MEAN_TEST_TYPES)

    value = str(test_type or "").strip().lower()
    aliases = {
        "student_t_test": "one_sample_t_test",
        "student_t": "one_sample_t_test",
        "one_sample_t": "one_sample_t_test",
        "welch": "welch_t_test",
        "welchs_t_test": "welch_t_test",
        "anova": "anova_test",
        "anova_one_way": "anova_test",
        "anova_oneway": "anova_test",
        "anova_test": "anova_test",
        "levene": "levene_test",
        "levene_test": "levene_test"
    }
    value = aliases.get(value, value)
    return value if value in allowed else default_test


def normalize_comparison_scope(scope_value, test_type, aggregation="mean"):
    value = str(scope_value or "").strip().lower()
    aliases = {
        "group_vs_global": "group_vs_global",
        "global": "group_vs_global",
        "vs_global": "group_vs_global",
        "group_vs_rest": "group_vs_rest",
        "rest": "group_vs_rest",
        "vs_rest": "group_vs_rest",
        "group_vs_group": "group_vs_group",
        "between_groups": "group_vs_group",
        "vs_group": "group_vs_group"
    }
    value = aliases.get(value, value)

    if aggregation == "variance":
        return value if value in {"group_vs_global", "group_vs_rest", "group_vs_group"} else "group_vs_rest"

    if test_type == "one_sample_t_test":
        return "group_vs_global"

    return value if value in {"group_vs_rest", "group_vs_group", "group_vs_global"} else "group_vs_rest"


def extract_valid_conditions(condition_list, allowed_columns, selected_dimension):
    valid = {}
    for c in condition_list or []:
        if not isinstance(c, dict):
            continue
        col = c.get("column")
        val = c.get("value")
        if col in allowed_columns and col != selected_dimension:
            valid[col] = val
    return valid


def format_conditions_text(conditions_dict):
    if not conditions_dict:
        return "the entire dataset"
    return " AND ".join([f"`{col}` = '{val}'" for col, val in conditions_dict.items()])


def normalize_generated_hypotheses(hyp_list):
    normalized = []
    for h in hyp_list:
        if not isinstance(h, dict):
            continue
        new_h = dict(h)
        aggregation = str(new_h.get("aggregation", "mean")).lower()
        new_h["aggregation"] = aggregation
        test_type = normalize_test_type(
            new_h.get("test_type") or new_h.get("test") or new_h.get("statistical_test"),
            aggregation
        )
        new_h["test_type"] = test_type
        new_h["comparison_scope"] = normalize_comparison_scope(
            new_h.get("comparison_scope") or new_h.get("scope"),
            test_type,
            aggregation
        )
...
def compress_advisor_text(text):
    return str(text or "").strip()


STATISTICAL_ADVISOR_PROMPT = """
You are a Statistical Test Advisor for exploratory data analysis.

Your role is to briefly talk to the user in natural language and recommend
which statistical tests are the best candidates for the user's goal.

You must:
- consider the user intent and data context, suggest appropriate statical tests from the available suite: 
one-sample t-test, Welch's t-test, ANOVA, and Levene's test.
- explain in simple language why each test is useful
- Write one paragraph at most. Limit your output to 200 tokens.

Important:

- Do not invent dataset properties not supported by DATA CONTEXT.
- Do NOT return JSON or Markdown .MD language.
- Write as if you are directly talking to the user inside the app UI.
- Do NOT use more than 200 tokens. Write all this information in one paragraph.
...
Your task is to generate **statistically testable hypotheses** that compare subgroups
using only the following statistical tests:
...
A **subgroup** is a subset of dataset rows defined by one or more **equality conditions on dataset columns**
(for example: `column == value`).

You **do not compute statistics or p-values**. Your role is only to propose candidate
hypotheses that define meaningful subgroups for later statistical testing.

If there is a user intention, guide your hypotheses toward solving it.

---

## Available Information

You only have access to summary information in **DATA CONTEXT**.
...
- Guide your hypotheses toward the user intention if provided.
- The plural of hypothesis is hypotheses. Do **not** use hypothesises.

---

## Allowed Test Usage

Use the tests as follows:

- **one_sample_t_test**:
  use for hypotheses where a subgroup mean is compared against the **global mean**
- **welch_t_test**:
  use for hypotheses where a subgroup mean is compared against the **rest of the dataset**
  or against another explicitly defined reference group
- **anova_test**:
  use only when the hypothesis is about comparing a subgroup against a reference defined by
  multiple groups or when the intent clearly suggests a broader group-comparison framing
- **levene_test**:
  use only for **variance** hypotheses

Do not use any other test names.

---

## Baseline

Each hypothesis must compare a subgroup against one of these references:

- the **global mean** of `{target_dimension}`
- the **rest of the dataset**
- an explicitly defined **reference group**
- the corresponding **variance** reference when using `levene_test`
...
{{
  ...
  "test_type": "one_sample_t_test" | "welch_t_test" | "anova_test" | "levene_test",
  "comparison_scope": "group_vs_global" | "group_vs_rest" | "group_vs_group",
  "conditions": [
...
- For `"aggregation": "variance"`, always use `"test_type": "levene_test"`.
- For `"test_type": "one_sample_t_test"`, use `"comparison_scope": "group_vs_global"`.
- For `"test_type": "welch_t_test"`, prefer `"comparison_scope": "group_vs_rest"` or `"group_vs_group"`.
- For `"test_type": "anova_test"`, use `"comparison_scope": "group_vs_rest"` or `"group_vs_group"`.
- If `"comparison_scope": "group_vs_global"`, `reference_conditions` should be an empty list.
- If `"comparison_scope": "group_vs_rest"`, `reference_conditions` should be an empty list.
- If `"comparison_scope": "group_vs_group"`, provide non-empty `reference_conditions`.
- Use only columns and values supported by DATA CONTEXT.
- Conditions must be interpretable and realistic.
- Avoid duplicates and near-duplicates of previous hypotheses.
...
- **Impact**: prioritize subgroups whose target statistic may meaningfully deviate from the reference
- **Homogeneity**: coherent, interpretable subgroups

Before generating hypotheses, identify which dataset dimensions are most relevant to the USER INTENT
and prioritize them.
...
def generate_statistical_advice(full_df, selected_dimension, params, job_id=None):
    ...
    try:
        ...
        prompt = PromptTemplate.from_template(STATISTICAL_ADVISOR_PROMPT)
        chain = prompt | llm

        if job_id:
            update_ai_progress(
                job_id,
                advisor_status="running",
                advisor_text="Analyzing your goal and selecting the most appropriate statistical tests..."
            )
            ...
            if chunk.content:
                ...
                if job_id:
                    update_ai_progress(
                        job_id,
                        advisor_status="running",
                        advisor_text="".join(response_chunks)
                    )

        final_text = compress_advisor_text("".join(response_chunks))

        if job_id:
            update_ai_progress(
                job_id,
                advisor_status="completed",
                advisor_text=final_text
            )

        return final_text

    except Exception as e:
        error_text = f"Could not generate statistical advice: {str(e)}"
        if job_id:
            update_ai_progress(
                job_id,
                advisor_status="completed",
                advisor_text=error_text
            )
        return error_text
...
def discover_with_ai_agent(full_df, discretized_df, original_df, selected_dimension, global_mean, params, job_id=None):
    ...
    if job_id:
        update_ai_progress(
            ...
            last_completed_run=0,
            phase="starting",
            advisor_status=AI_PROGRESS.get(job_id, {}).get("advisor_status", "pending"),
            advisor_text=AI_PROGRESS.get(job_id, {}).get("advisor_text", "")
        )
            ...
            try:
                ...
                for chunk in chain.stream({
                    ...
                    if chunk.content:
                        ...
                        if job_id:
                            current_raw = "".join(response_chunks)
                            current_preview = _safe_load_json_array(current_raw)
                            update_ai_progress(
                                job_id,
                                status="running",
                                phase="streaming",
                                message=f"Generating hypotheses... Run {i} of {total_iterations}.",
                                progress=max(1, int(((i - 1) / max(total_iterations, 1)) * 100)),
                                current_run=i,
                                total_runs=total_iterations,
                                live_raw_text=current_raw,
                                live_run_hypotheses=current_preview,
                                live_hypotheses=memory + current_preview,
                                last_completed_run=i - 1
                            )

                last_raw = "".join(response_chunks)
                print("\n")

                parsed_preview = _safe_load_json_array(last_raw)
                debug_logs["runs"].append({
                    "run": i,
                    "attempt": attempt,
                    "raw_response": last_raw,
                    "parsed_preview_count": len(parsed_preview),
                    "parsed_preview": parsed_preview[:5]
                })

                parsed = normalize_generated_hypotheses(parsed_preview)
                ...
                current_user_intent = (
                    ...
                    + f"\n\n(Repair instruction: you returned too few NEW hypotheses. "
                      f"Generate up to {min_hypotheses_per_run} NEW hypotheses NOT present in PREVIOUS_HYPOTHESES_JSON. "
                      f"If you cannot generate {min_hypotheses_per_run}, still return all valid NEW hypotheses you can find.)"
                )
            except Exception as e:
        ...
        if job_id:
            ...
            if i < total_iterations:
                update_ai_progress(
                    ...
                    status="running",
                    phase="run_completed",
                    message=f"Run {i} of {total_iterations} completed. Starting run {i+1}...",
                    progress=progress_pct,
                    current_run=i,
                    total_runs=total_iterations,
                    live_raw_text="",
                    live_run_hypotheses=[],
                    live_hypotheses=memory,
    ...
    for h in memory:
        ...
        ref_conds_list = h.get("reference_conditions", [])

        valid_conds = extract_valid_conditions(conds_list, discretized_df.columns, selected_dimension)
        reference_conds = extract_valid_conditions(ref_conds_list, discretized_df.columns, selected_dimension)

        if valid_conds:
            normalized_test = normalize_test_type(h.get("test_type"), agg)
            comparison_scope = normalize_comparison_scope(h.get("comparison_scope"), normalized_test, agg)

            if comparison_scope == "group_vs_group" and not reference_conds:
                comparison_scope = "group_vs_rest"
            ...
            formatted_hyps.append({
                ...
                "Reference_Conditions": reference_conds,
                "Comparison_Scope": comparison_scope,
                "Test_Type": normalized_test
            })
    ...
    def format_ai_hypothesis_text(row):
        ...
        comparison_scope = row.get("Comparison_Scope", "group_vs_global")
        reference_conds = row.get("Reference_Conditions", {}) or {}

        if comparison_scope == "group_vs_global":
            reference_label = "the global dataset"
        elif comparison_scope == "group_vs_rest":
            reference_label = "the rest of the dataset"
        elif comparison_scope == "group_vs_group" and reference_conds:
            ref_parts = [f"`{col}` = '{val}'" for col, val in reference_conds.items()]
            reference_label = "the reference group defined by " + " AND ".join(ref_parts)
        else:
            reference_label = "the reference group"

        if 'variance' in row['Operator']:
            return (
                f"Claim: The group defined by {region_str} exhibits a **higher variance** "
                f"for `{row['Dimension']}` compared to {reference_label}."
            )
        else:
            return (
                f"Claim: The group defined by {region_str} has a mean for `{row['Dimension']}` "
                f"that is **{row['Operator']}** {reference_label}."
            )
    ...
    sig_count = int(ranked_df['is_significant'].sum()) if 'is_significant' in ranked_df.columns else len(ranked_df)
    print(f"   -> {sig_count} hypotheses marked as statistically significant before route-level filtering.")
    ...
    if job_id:
        AI_PROGRESS[job_id] = {
            ...
            "total_runs": total_iterations,
            "advisor_status": AI_PROGRESS.get(job_id, {}).get("advisor_status", "completed"),
            "advisor_text": AI_PROGRESS.get(job_id, {}).get("advisor_text", "")
        }
    ...
    for method_key, params in payload.methods.items():
        ...
        if method_key == 'ga':
            ...
            if not result_df.empty:
                if 'Test_Type' not in result_df.columns:
                    result_df['Test_Type'] = result_df['Operator'].apply(lambda x: normalize_test_type(None, 'variance' if 'variance' in str(x) else 'mean'))
            ...
            if payload.comparison_column_indices:
                ...
                if all_welchs_results:
                    ...
                    result_df['Test_Type'] = "welch_t_test"
    ...
    if not all_hypotheses_df.empty:
        ...
        def create_canonical_key(row):
            if 'Conditions' in row and isinstance(row.get('Conditions'), dict):
                cond_items = sorted(row['Conditions'].items())
                ref_items = sorted((row.get('Reference_Conditions') or {}).items()) if isinstance(row.get('Reference_Conditions'), dict) else []
                return str((
                    cond_items,
                    row.get('Operator'),
                    row.get('Test_Type'),
...
def generate_ai_async(payload: TablePayload):
    ...
    def run_job():
        try:
            ...
            advisor_text = generate_statistical_advice(
                df,
                selected_dimension,
                params,
                job_id=job_id
            ...
            AI_RESULTS[job_id] = {
                "run_params": {
                    ...
                    "selected_dimension": selected_dimension,
                    "global_mean": global_mean,
                    "statistical_advice": advisor_text
                },
...
@app.post("/generate_statistical_advice_stream")
async def generate_statistical_advice_stream(payload: TablePayload):
    ...
    def stream():
        try:
            ...
            if payload.methods and isinstance(payload.methods, dict):
                ai_params = payload.methods.get("ai_agent", payload.methods)
                provider = ai_params.get("provider", "OpenAI")
                api_key = ai_params.get("api_key", "")
                model_name = ai_params.get("model_name", "gpt-5.4-mini")
                temperature = float(ai_params.get("temperature", 0.1))

            llm = build_llm(provider, model_name, temperature, api_key)

            df = pd.DataFrame(payload.rows, columns=payload.headers)
            data_context = utils.describe_dataset_detailed(df)

            prompt = STATISTICAL_ADVISOR_PROMPT
            ...
            # força 1 parágrafo curto mesmo se o modelo exceder
