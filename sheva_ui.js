...
let advisorAlreadyGenerated = false;

window.lastAdvisorQuery = '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    csvFileInput.addEventListener('change', (event) => {
        if (event.target.files && event.target.files[0]) {
            ...
            window.ShevaCore.loadCSVFile(file, (filename) => {
                // Novo dataset: não preservar description anterior
                renderTable(filename, false);
            });
        }
    });
    ...
    let existingToast = document.getElementById('success-toast');
    if (existingToast) {
    ...
    // --- AI PROGRESS BAR + LIVE HYPOTHESIS PREVIEW ---
    ...
    function ensureAIProgressBox(resultsArea) {
        ...
        if (!existingProgressBox) {
            ...
            progressWrapper.innerHTML = `
                <div id="ai-progress-box" class="hidden p-4 border rounded-lg bg-white mt-4 shadow-sm">
                    <div class="flex justify-between items-center mb-2">
                        <span id="ai-progress-text" class="text-sm text-gray-700 font-medium">
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-3 overflow-hidden mb-4">
                        <div id="ai-progress-fill" class="h-3 bg-indigo-600 transition-all duration-300" style="width: 0%;"></div>
                    </div>

                    <div id="ai-live-preview-wrapper" class="space-y-4">

                        <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-semibold text-emerald-900 flex items-center">
                                        <i class="fas fa-flask mr-2"></i>
                                        Statistical test advisor
                                    </h4>
                                    <span id="ai-advisor-status" class="text-xs font-medium bg-emerald-100 text-emerald-800 px-2 py-1 rounded-full">
                                        Pending
                                    </span>
                                </div>
                                <div id="ai-advisor-text" class="text-sm text-emerald-900 whitespace-pre-wrap">
                                    Waiting for test recommendations...
                                </div>
                            </div>
                        <div class="rounded-xl border border-amber-200 bg-amber-50 p-4">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span id="ai-live-run-badge" class="text-xs font-medium bg-amber-100 text-amber-800 px-2 py-1 rounded-full">
                                    Run 0 / 0
                                </span>
                            </div>
                            <p class="text-sm text-amber-800">
                                The model is currently working on generating possible hypotheses.
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            <div class="rounded-xl border border-gray-200 bg-gray-50 p-4">
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-semibold text-gray-800">Live raw output</h4>
                                    <span class="text-xs text-gray-500">Streaming preview</span>
                                </div>
                                <pre id="ai-live-raw-output" class="text-xs text-gray-700 whitespace-pre-wrap break-words max-h-80 overflow-y-auto bg-white p-3 rounded-lg border border-gray-200">Waiting for model output...</pre>
                            </div>

                            <div class="rounded-xl border border-indigo-200 bg-indigo-50 p-4">
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="font-semibold text-indigo-900">Parsed hypotheses preview</h4>
                                    <span id="ai-live-hyp-count" class="text-xs font-medium bg-indigo-100 text-indigo-800 px-2 py-1 rounded-full">
                                        0 hypotheses
                                    </span>
                                </div>
                                <div id="ai-live-hypotheses-list" class="space-y-2 max-h-80 overflow-y-auto">
                                    <p class="text-sm text-gray-500">No parsed hypotheses yet.</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    function hideAIProgressBox() {
        ...
        const progressText = document.getElementById('ai-progress-text')?.parentElement?.parentElement;
        const hypothesesBanner = progressBox.querySelector('.rounded-xl.border.border-amber-200');
        const previewGrid = progressBox.querySelector('.grid.grid-cols-1.lg\\:grid-cols-2.gap-4');

        if (progressText) progressText.classList.add('hidden');
        if (hypothesesBanner) hypothesesBanner.classList.add('hidden');
    ...
    function formatConditionList(conditions = {}) {
        ...
        if (!entries.length) return 'No conditions';

        return entries.map(([col, val]) => `${col} = "${val}"`).join(' AND ');
    }

    function updateLiveHypothesisPreview(progressData = {}) {
        const rawOutputEl = document.getElementById('ai-live-raw-output');
        const hypListEl = document.getElementById('ai-live-hypotheses-list');
        const hypCountEl = document.getElementById('ai-live-hyp-count');
        const runBadgeEl = document.getElementById('ai-live-run-badge');

        const advisorTextEl = document.getElementById('ai-advisor-text');
        const advisorStatusEl = document.getElementById('ai-advisor-status');

        if (!rawOutputEl || !hypListEl || !hypCountEl || !runBadgeEl) return;

        const currentRun = progressData.current_run ?? 0;
        const totalRuns = progressData.total_runs ?? 0;
        runBadgeEl.textContent = `Run ${currentRun} / ${totalRuns}`;

        const rawText = progressData.live_raw_text || '';
        rawOutputEl.textContent = rawText.trim() || 'Waiting for model output...';

        if (advisorTextEl && progressData.advisor_text !== undefined) {
            advisorTextEl.textContent = progressData.advisor_text;
        }

        if (advisorStatusEl && progressData.advisor_status !== undefined) {
            const status = progressData.advisor_status;
        ...
        let hypotheses = [];
        if (Array.isArray(progressData.live_hypotheses) && progressData.live_hypotheses.length > 0) {
            hypotheses = progressData.live_hypotheses;
        } else if (Array.isArray(progressData.live_run_hypotheses) && progressData.live_run_hypotheses.length > 0) {
            hypotheses = progressData.live_run_hypotheses;
        }

        hypCountEl.textContent = `${hypotheses.length} hypotheses`;

        if (!hypotheses.length) {
            hypListEl.innerHTML = `<p class="text-sm text-gray-500">No parsed hypotheses yet.</p>`;
            return;
        }

        hypListEl.innerHTML = hypotheses.map((hyp, idx) => {
            const aggregation = hyp.aggregation || 'mean';
            const comparison = hyp.comparison || '?';
            const targetMetric = hyp.target_metric || 'target';
            const testType = hyp.test_type || hyp.Test_Type || 'unknown';
            const referenceLabel = testType === 'one_sample_t_test' ? 'global mean' : 'reference';
            const conditions = Array.isArray(hyp.conditions)
                ? hyp.conditions.reduce((acc, c) => {
                    if (c?.column) acc[c.column] = c.value;
                    return acc;
                }, {})
                : {};
                ...
                <div class="bg-white border border-indigo-100 rounded-lg p-3 shadow-sm">
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <span class="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded-full">
                            ${aggregation} ${comparison} ${referenceLabel}
                        </span>
                    </div>
                    <p class="text-sm text-gray-800 mb-1">
                        <strong>${targetMetric}</strong> with <strong>${aggregation}</strong> <strong>${comparison}</strong> ${referenceLabel}
                    </p>
    ...
    async function pollAIProgress(jobId) {
        while (true) {
            ...
            updateLiveHypothesisPreview(data);
    ...
    async function startAdvisorStreaming(payload) {
        const advisorTextEl = document.getElementById('ai-advisor-text');
        const advisorStatusEl = document.getElementById('ai-advisor-status');

        if (!advisorTextEl || !advisorStatusEl) return;

        advisorTextEl.textContent = '';
        advisorStatusEl.textContent = 'Analyzing';

        try {
            const response = await fetch('http://localhost:8000/generate_statistical_advice_stream', {
                method: 'POST',
            ...
            while (true) {
                ...
                advisorTextEl.textContent = fullText;
            }

            advisorStatusEl.textContent = 'Ready';

        } catch (error) {
            advisorTextEl.textContent = 'Error generating advice.';
            advisorStatusEl.textContent = 'Error';
        }
    }
    ...
    async function runAIHypothesisGeneration(payload) {
        ...
        if (!advisorAlreadyGenerated) {
            advisorAlreadyGenerated = true;
            startAdvisorStreaming(payload);
        }
     ...
     * @param {boolean} preserveDescription - Whether to preserve the current dataset description textarea value.
     */
    function renderTable(fileName, preserveDescription = true) {
        saveCollapsibleStates();

        // Preserva a descrição somente em rerenders internos do mesmo dataset.
        // Em carregamento de novo CSV, preserveDescription deve ser false.
        if (preserveDescription) {
            ...
            window.ShevaCore.state.datasetDescriptionText = '';
        }
    ...
    function addEventListeners() {
        ...
        document.querySelectorAll('.delete-icon').forEach(icon =>
            icon.addEventListener('click', (e) => {
                ...
                renderTable(csvFileInput.files[0]?.name || 'data', true);
            })
        );
    ...
    const generateHypotheses = (generationParams, appendHypotheses = false) => {
        ...
        if (description && description !== window.lastAdvisorQuery) {
            advisorAlreadyGenerated = false;
            window.lastAdvisorQuery = description;
        }
        ...
        requestPromise
            .then(data => {
                ...
                let persistedAdvisorHTML = '';
                    const existingAdvisorBox = document.getElementById('ai-progress-box');

                    if (existingAdvisorBox) {
                        const advisorCard = existingAdvisorBox.querySelector('.rounded-xl.border.border-emerald-200');
                        if (advisorCard) {
                            persistedAdvisorHTML = advisorCard.outerHTML;
                        }
                    }
                if (!appendHypotheses || !document.getElementById('tab-table')) {
                    resultsArea.innerHTML = `
                     ${persistedAdvisorHTML}
                ...
                if (data && data.final_hypotheses_df) {
                    ...
                    const validCount = Array.isArray(hypotheses) ? hypotheses.length : 0;

                    if (validCount > 0) {
                        showSuccessToast(`Generation completed: ${validCount} significant hypotheses loaded in Table View.`);
                    } else {
                        showSuccessToast(`Generation completed: no significant hypotheses were found.`);
                    }
