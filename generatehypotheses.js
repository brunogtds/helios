...
function getReadableTestName(testType) {
    const map = {
    ...
    return map[testType] || testType || "Unknown test";
}

function buildHypothesisCategoryKey(hyp) {
    const operator = hyp.Operator || "unknown";
    const testType = hyp.Test_Type || "unknown_test";
    return `${operator}__${testType}`;
}

function buildHypothesisCategoryTitle(hypotheses) {
    if (!hypotheses || !hypotheses.length) return "Hypotheses";

    const sample = hypotheses[0];
    const operator = sample.Operator || "unknown";
    const testName = getReadableTestName(sample.Test_Type);

    if (operator === "greater than") {
        return `Hypotheses: Higher than reference (${testName})`;
    }
    if (operator === "less than") {
        return `Hypotheses: Lower than reference (${testName})`;
    }
    if (operator === "variance is higher") {
        return `Hypotheses: Higher variance (${testName})`;
    }

    return `Hypotheses: ${operator} (${testName})`;
}
            ...
            <div class="bg-gray-50 p-4 rounded-lg">
                <div class="flex items-start justify-between space-x-4 mb-4">
                    <p class="text-sm text-gray-600 mt-1 flex-grow">
                        The AI will analyze your generated hypotheses. It will also provide a summary of key findings and suggest next steps for your analysis.
                    </p>
...
function setupAndRenderHypotheses(hypotheses, sunburstJson, newTargetInfo, tableData) {
    ...
    sortedHypotheses.forEach(h => {
        const key = buildHypothesisCategoryKey(h);
        if (!grouped[key]) grouped[key] = [];
        grouped[key].push(h);
    });

    categorizedHypotheses = grouped;
    ...
    window.hypothesesApp.getReadableTestName = getReadableTestName;
    window.hypothesesApp.buildHypothesisCategoryTitle = buildHypothesisCategoryTitle;
